...
static void __view_event_handler(void *handler_args, esp_event_base_t base, int32_t id, void *event_data)
{
    ...
    case VIEW_EVENT_WIFI_LIST_REQ: {
        ...
        ap_count = min_int(number, ap_count);
...
{
    if (!__g_data_mutex || !view_event_handle) {
        return;
    }
    struct view_data_wifi_st st;
    __wifi_st_get(&st);
    esp_err_t err = esp_event_post_to(view_event_handle, VIEW_EVENT_BASE, VIEW_EVENT_WIFI_ST, &st, sizeof(st), portMAX_DELAY);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to post wifi status: %s", esp_err_to_name(err));
    }
}
